...
def handle_position_opened(event: PositionOpenedEvent):
    ...
    try:
        ...
        task = loop.create_task(_send_opened(event))
    except RuntimeError:
...
def handle_position_closed(event: PositionClosedEvent):
    ...
    try:
        loop = asyncio.get_running_loop()
        task = loop.create_task(_send_closed(event))
    except RuntimeError:
...
def setup_telegram_subscribers():
    bus.subscribe("position.opened", handle_position_opened, "telegram_pos_opened")
    bus.subscribe("position.closed", handle_position_closed, "telegram_pos_closed")
    logger.info("Telegram event subscribers registered on the EventBus.")
