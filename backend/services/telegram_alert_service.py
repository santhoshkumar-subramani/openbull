...
class TelegramAlertService:
    ...
    @staticmethod
    def _get_config(db: Session, user_id: int) -> Optional[TelegramConfig]:
        return db.query(TelegramConfig).filter(TelegramConfig.user_id == user_id).first()
    ...
    @classmethod
    def dispatch_alert(cls, db: Session, user_id: int, message: str):
        """
        Dispatches an alert in the background. Does not block the main thread.
        """
        config = cls._get_config(db, user_id)
    ...
    @classmethod
    def send_order_alert(cls, db: Session, user_id: int, order_details: dict):
        ...
        cls.dispatch_alert(db, user_id, message)

    @classmethod
    def send_trade_alert(cls, db: Session, user_id: int, trade_details: dict):
